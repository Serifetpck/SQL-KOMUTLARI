...
-- SORU:satılan meyve türüne (urun_adi) göre urun alan kişi sayısını gösteren
-- sorguyu yazınız. NULL olarak girilen meyveyi listelemesin.
SELECT urun_adi, COUNT(isim) as kisi_sayisi from manav
where urun_adi is not null
group by urun_adi

--SORU: satılan meyve türüne (urun_adi) göre satılan (urun_miktari )MIN ve 
...
-- SORU:kisi ismine ve urun adına göre satılan ürünlerin toplamını gruplandıran 
...
-- SORU:Personeli, ulkelere göre ve şehirlere göre gruplayarak sorgulayın
SELECT ulke,sehir from personel
group by ulke,sehir

-- SORU:Personelin, ulkelere göre ve şehirler göre calışan sayısını sorgulayın.
...
-- SORU: Her ulke için bay ve bayan çalışan sayısı ve yaş ortalamasını ve maası 30000 den büyük olanları sorgulayınız.
SELECT ulke, cinsiyet, round(avg(yas)) as ortalama_yas, count(*) from personel
WHERE maas>30000
group by ulke,cinsiyet

-- SORU: Her ulke için; bay ve bayan çalışan sayısı, yaş ortalamasını, maaşı 30000 den büyük olanları
...
--SORU:Maaş ortalaması 3000’den fazla olan ülkelerdeki erkek çalışanların maaş ortalaması.
SELECT ulke, round(avg(maas)) as maas_ortalamasi from personel
WHERE cinsiyet='E'
group by ulke 
having avg(maas)>3000

--SORU:-Erkek çalışanların sayısı 1’den fazla olan ülkelerin maaş ortalamasını getiren sorgu
